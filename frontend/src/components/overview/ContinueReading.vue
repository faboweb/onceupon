...
<div style="display: flex; flex-direction: column" v-if="continueStore.proposals.length">
    <div style="display: flex; justify-content: space-between; margin-bottom: 1rem;     margin-top: 1rem;">
        <span>Continue where you left off</span>
    </div>
    <div>
        <div v-for="proposal, i of continueStore.proposals" :key="i" style="background: rgba(217, 217, 217, 0.2); border-radius: 0 4px 4px 0; padding: 0.5rem; cursor: pointer; width: 154px; height: 100px"
        @click="router.push('/story/' + proposal.storyId + '/read/')">
            <b style="margin-bottom: 0.2rem; font-size: 14px; white-space: nowrap;">
            {{ proposal.title }}
            </b>
            <span style="color: rgba(242, 103, 9, 0.6); font-size: 14px">Proposal</span>
            <p style="text-align: left; white-space: pre-wrap; text-overflow: ellipsis;
        overflow: hidden; max-height: 70px; font-size: 12px">
...
<div v-else>
    ...
    <div v-if="randomStoryEnding">
        <div style="background: rgba(217, 217, 217, 0.2); border-radius: 0 4px 4px 0; padding: 0.5rem; display: flex; flex-direction: column; cursor: pointer; width: 186px; height: 123px; overflow: hidden; text-overflow: ellipsis;"
        @click="router.push('/story/' + randomStory.id + '/read/')">
            <b style="margin-bottom: 0.2rem; font-size: 12px; white-space: nowrap; ">
            {{ randomStory.name }}
            </b>
            <span style="color: rgba(0, 128, 0, 0.6); font-size: 12px">Continue the story</span>
            <p style="text-align: left; white-space: pre-wrap; text-overflow: ellipsis;
        overflow: hidden; max-height: 70px; font-size: 12px">
            {{ randomStoryEnding }}
            </p>
    ...
    <div v-else>
        <ion-skeleton-text style="height: 123px; width: 186px; border-radius: 4px" :animated="true">
        </ion-skeleton-text>
    </div>
</div>
</template>
...
watch(randomStory, async () => {
    const lastSection = randomStory.value.sections[randomStory.value.sections.length - 1]
...
watch(() => storyStore.stories, () => {
  ...
  randomStoryIndex.value = Math.floor(Math.random() * storyStore.stories.length)
    const story = storyStore.stories[randomStoryIndex.value]
  if (story) {
    storyStore.getStory(story.id).then(story => randomStory.value = story)
  }
}, {
    immediate: true
})
